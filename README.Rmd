...
An R package providing color palettes for pro sports teams. The palettes are provided by [Jim Neilsen's Team Colors website](http://jim-nielsen.com/teamcolors/) and offered with only minimal alterations. NCAA colors come [teamcolorcodes.com](https://teamcolorcodes.com/), via the [ncaahoopR R Package](https://github.com/lbenz730/ncaahoopR). Note that hexcodes are only available for 248 of 353 Division 1 teams.
...
To view color palettes for college teams, simply call the `show_ncaa_col()` function.

```{r ncaacolors, fig.height=10, fig.width=10}
show_ncaa_col()
```
